...
// frappe.ui.form.on("Salary Slip Leave Balance", "leave_allocated", function (frm, cdt, cdn){
//  		var d = locals[cdt][cdn];
//  		console.log(d.leave_type)
// 			frappe.call({
...
// 				},
// 				callback: function(r) {
// 					frappe.model.set_value(cdt, cdn, "leave_balance", r.message);
// 				}
// 			});
// });

frappe.ui.form.on("Salary Slip", "employee", function one(frm){
	var leave_type = frappe.call({
					...
					callback: function(r){
							...
							for(var e=1; e<r.message.length+1; e++) {
					        frappe.model.add_child(cur_frm.doc, "Salary Slip Leave Balance", "table_55");  
					        $.each(frm.doc.table_55 || [], function(e, v) {	
					        frappe.model.set_value(v.doctype, v.name, "leave_type", r.message[e].leave_type)
					        frappe.model.set_value(v.doctype, v.name, "leave_allocated", r.message[e].total_leaves_allocated)
							frappe.ui.form.on("Salary Slip Leave Balance", "leave_allocated", function (frm, cdt, cdn){
							 		var d = locals[cdt][cdn];
							 		// console.log(d.leave_type)
										frappe.call({
											...
											callback: function(r) {
												frappe.model.set_value(cdt, cdn, "leave_balance", r.message);
												frappe.model.set_value(cdt, cdn, "leave_taken", parseInt(d.leave_allocated)-parseInt(d.leave_balance));
											}
										});
							});						
					        })
					        frm.refresh_field("table_55");	
