...
def get_children(doctype, parent, task=None, project=None, is_root=False):
	...
	if task:
		# via filters
		conditions += ' and parent_item = "{0}"'.format(frappe.db.escape(task))
	elif parent and not is_root:
		# via expand child
		conditions += ' and parent_item = "{0}"'.format(frappe.db.escape(parent))
	else:
		conditions += ' and ifnull(parent_item, "")=""'

	tasks = frappe.db.sql("""select item_name as value,
		item_code as title,
		is_group as expandable
		from `tabItem`
		where docstatus < 2
		{conditions}
		order by item_name""".format(conditions=conditions), as_dict=1)
...
@frappe.whitelist()
def add_multiple_tasks(data, parent, is_group, element_type, item_group, is_stock_item,is_fixed_asset, asset_category, stock_uom):
	data = json.loads(data)['items']	
	items = data.split('\n')
	if parent == 'All Items' :
		parent = None	

	# parents = get_parents(element_type, parent)	

	new_doc = {'doctype': 'Item', 'parent_item': parent, 'item_group': item_group, 'is_fixed_asset': int(is_fixed_asset), 
			   'asset_category': asset_category, 'is_group': is_group, 'type': element_type, 'stock_uom': stock_uom,
			   'is_stock_item': int(is_stock_item)
			   # 'functional_block': parents["functional"], 'subfunctional_block': parents[sub_functional],
			   # 'equipment_block':  parents[equipment]
			  }
	...
	for d in items:
		...
		# new_docc = {'doctype': 'Asset', 'item_code': d, 'asset_name': d}
